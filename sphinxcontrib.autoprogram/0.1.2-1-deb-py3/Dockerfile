...
LABEL    software="sphinxcontrib.autoprogram" \ 
    ...
    software.version="0.1.2-1-deb-py3" \ 
    version="1" \ 
...
RUN apt-get update && apt-get install -y python3-sphinxcontrib.autoprogram && apt-get clean && apt-get purge && rm -rf /var/lib/apt/lists/* /tmp/*
USER biodocker
